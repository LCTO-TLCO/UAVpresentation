...
scenario = "Passive Auditory Oddball MMN";
...
int PORT_CODE_INIT_STD = 180;
int PORT_CODE_STD = 80;
int PORT_CODE_DEV = 70;
...
if ( parameter_manager.get_bool( "Generate Sounds" ) ) then
	...
	double ramp_up_time = parameter_manager.get_double( "Generated Sound Rise Time" );
	double ramp_down_time = parameter_manager.get_double( "Generated Sound Fall Time" );
	...
	double std_duration = parameter_manager.get_double( "Generated Sound Standard Duration" );
	double dev_duration = parameter_manager.get_double( "Generated Sound Deviant Duration" );
	asg::sine std_data = new asg::sine( std_duration, parameter_manager.get_double( "Generated Sound Standard Frequency" ), 0.0 );
	asg::sine dev_data = new asg::sine( dev_duration, parameter_manager.get_double( "Generated Sound Deviant Frequency" ), 0.0 );
	...
	if ( ramp_time > std_duration ) || ( ramp_time > dev_duration ) then
		exit( "Error: The total rise and fall time is greater than the sound duration." );
	end;
...
int min_start = parameter_manager.get_int( "Pre-Task Standards" );

begin
	# Get some trial counts
	int total_trials = parameter_manager.get_int( "Total Stimuli" );
	int std_trials = int( ceil( parameter_manager.get_double( "Standard Stimulus Proportion" ) * double(total_trials) ) );
	int dev_trials = total_trials - std_trials;

	# Figure out some of the restrictions
	int min_between = parameter_manager.get_int( "Minimum Standards Between Deviants" );

	# Make sure there are enough trials to meet the restrictions
	int set_aside_stds = min_start + ( dev_trials * min_between );
	int remaining_stds = std_trials - set_aside_stds;
	if ( remaining_stds < 0 ) then
		exit( "Error: Not enough standard trials to create a legal sequence with the current settings." );
	end;
...
port_codes[STD_IDX] = PORT_CODE_STD;
port_codes[DEV_IDX] = PORT_CODE_DEV;
