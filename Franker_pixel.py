...
for i in range(10):
    tmp = scen.text()
    tmp.set_caption(str(i + 1) + "/10ブロック目")
    tmp.set_font_size(36)
    block_letters.append(tmp)
    tmp_tale = scen.text()
    tmp_tale.set_caption(str(i + 1) + "/10ブロック目終了")
...
text_block_tail.set_caption(str(block_count) + "ブロック目終了")
text_block_tail.set_font_size(36)
text_block_tail.load()
pic_block_tail.add_part(text_block_tail, origin_x=0, origin_y=0)
text_block_tail_settings = scen.text()
text_block_tail_settings.set_caption("設定を読み込みます。ボタンを押してしばらくお待ちください")
...
good.load()
oops = scen.text()
oops.set_caption("不正解")
oops.load()
missed = scen.text()
missed.set_caption("もっと速く！")
missed.load()
other = scen.text()
other.set_caption("other")
other.load()
