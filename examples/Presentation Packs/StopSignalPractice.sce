...
trial{
	...
	stimulus_event{
		...
		stimulus_time_out = $resp_wait_duration;
	}event_ss_l_mid;
...
loop int j=1 until j>select_trial.count() begin
	...
	elseif(select_trial[j]==5)then#--------------------------------------------------------------------------------5:SS_lm;
		...
		elseif stop.type() == stimulus_other then #Stop Signal Presentation
			sound_event_ss_l_mid.set_event_code("SS ssd " + string(SS_ssd));
			#change trial start
	...
	elseif(select_trial[j]==6)then#--------------------------------------------------------------------------------6:sc_li;
		...
		if stop.type() == stimulus_false_alarm then
			feedback_pic.set_part(1, f_a);		#debug feedback
			feedback_trial.present();				#debug feedback
			event_t_error_inhibit.set_event_code("inhibition error hc ssd" + string(SS_ssd));
			trial_t_error_inhibit.present();
		elseif stop.type() == stimulus_other then
			#set trigger
			sound_event_ss_l_idx.set_event_code("SS ssd " + string(SS_ssd));
			#change trial start
		...
		elseif stop.type() == stimulus_other then
			sound_event_ss_r_idx.set_event_code("fc ssd " + string(SS_ssd));
			#change trial start
		...
		elseif stop.type() == stimulus_other then
			sound_event_ss_r_mid.set_event_code("SS ssd " + string(SS_ssd));
			#change trial start
